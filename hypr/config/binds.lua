...
-- Move active window to a specific workspace (was SUPER+SHIFT+CONTROL+number)
for i = 1, NUM_WPM do
    local key = i % 10
...
hl.bind("SUPER + ALT + W", hl.dsp.exec_cmd("noctalia msg panel-toggle noctalia/mpvpaper:picker"))
...
-- Top bar
hl.bind(mainMod .. " + H", hl.dsp.exec_cmd(noctCall .. "bar-toggle"))
...
-- Focus on monitors
-- NOTE: commented out because SUPER+1/2/3 now switch workspaces instead (see below).
-- You're single-screen on the USB setup right now, so these weren't doing anything.
-- If you plug in an external monitor and want this back, rebind it to a free combo, e.g.:
-- hl.bind(mainMod .. " + SHIFT + M1", hl.dsp.focus({ monitor = MONITOR1 }))
...
-- Switch to workspace on current monitor (was SUPER+number)
for i = 1, NUM_WPM do
    local key = i % 10
